"""
alex_xrd_analysis.py — Alexandria XRD → AtomGPT.org Structure Reconstruction Benchmark

Benchmarks the AGAPI-XRD pipeline on structures from the Alexandria database.
For each entry: extracts lattice parameters and stoichiometry (leaving out atomic
coordinates), pushes the incomplete structure through the AGAPI-XRD pipeline, and
compares the reconstructed structure against the known DFT reference structure.

Usage:
  python alex_xrd_analysis.py
  python alex_xrd_analysis.py --max-entries 50 --method both --delay 3
  python alex_xrd_analysis.py --dataset-path /path/to/alex_pbe_hull

Jupyter:
  cfg = dict(DEFAULTS)
  cfg["max_entries"] = 50
  cfg["dataset_path"] = "./xrd_datasets/alex_pbe_hull"
...
DEFAULTS = dict(
    ...
    max_backoff=120.0, output_csv="alex_xrd_results.csv",
    output_poscars="alex_matched_poscars.json",
    cache_dir=".alex_cache", resume=True, overwrite_cache=False,
    write_every=1, max_abc=None,
)
...
def format_alex_xrd_data(intensities, two_theta_grid):
    return "\n".join(f"{t:.5f} {i:.6f}" for t, i in zip(two_theta_grid, intensities))

def load_alex_dataset(path_or_name, offset=0, max_entries=None):
    """Load Alexandria dataset from local parquet files or HuggingFace hub."""
    two_theta_grid = np.linspace(5.0, 90.0, 8501)

    if os.path.isdir(path_or_name):
        meta_path = os.path.join(path_or_name, "metadata.json")
        if os.path.isfile(meta_path):
            try:
                with open(meta_path) as f:
                    head = f.read(512)
                w_m = re.search(r'"wavelength"\s*:\s*([\d.]+)', head)
                min_m = re.search(r'"two_theta_min"\s*:\s*([\d.]+)', head)
                max_m = re.search(r'"two_theta_max"\s*:\s*([\d.]+)', head)
                step_m = re.search(r'"step"\s*:\s*([\d.]+)', head)
                if all([w_m, min_m, max_m, step_m]):
                    tmin, tmax, step = float(min_m.group(1)), float(max_m.group(1)), float(step_m.group(1))
                    n_pts = round((tmax - tmin) / step) + 1
                    two_theta_grid = np.linspace(tmin, tmax, n_pts)
                    print(f"XRD grid: wavelength={float(w_m.group(1))}Å, 2θ=[{tmin}°,{tmax}°], step={step}°, points={n_pts}")
            except Exception as e:
                print(f"Warning: could not parse metadata.json ({e}), using defaults.")

        parquet_files = sorted([
            os.path.join(path_or_name, f)
            for f in os.listdir(path_or_name)
            if f.endswith(".parquet")
        ])
        if not parquet_files:
            raise FileNotFoundError(f"No parquet files found in {path_or_name}")
        print(f"Loading {len(parquet_files)} parquet shards from {path_or_name}")
        from datasets import load_dataset
        ds = load_dataset("parquet", data_files={"train": parquet_files}, split="train")
    else:
        from datasets import load_dataset
        print(f"Loading dataset from HuggingFace hub: {path_or_name}")
        ds = load_dataset(path_or_name, split="train")

    total = len(ds)
    print(f"Total records: {total}")
    if offset > 0:
        ds = ds.select(range(offset, total))
        print(f"Skipping first {offset} entries (offset={offset})")
    if max_entries is not None and max_entries > 0:
        ds = ds.select(range(min(max_entries, len(ds))))
        print(f"Limiting to {max_entries} entries")

    return ds, two_theta_grid

def parse_alex_entry(record):
    """Extract formula, elements, reference cell, and XRD intensities from a record."""
    atoms_data = json.loads(record["atoms_json"]) if isinstance(record["atoms_json"], str) else record["atoms_json"]
    formula = record["formula"]
    elements = sorted(set(atoms_data["elements"]))
    abc = atoms_data["abc"]
    angles = atoms_data["angles"]
    volume = abs(float(np.linalg.det(np.array(atoms_data["lattice_mat"]))))
    ref_cell = {
        "a": float(abc[0]), "b": float(abc[1]), "c": float(abc[2]),
        "alpha": float(angles[0]), "beta": float(angles[1]), "gamma": float(angles[2]),
        "volume": volume,
    }
    return {
        "formula": formula,
        "elements": elements,
        "ref_cell": ref_cell,
        "intensities": np.array(record["xrd_intensities"], dtype=np.float64),
    }
...
def simulate_reconstructed_xrd(atoms):
    """TODO: Simulate XRD pattern of the reconstructed structure."""
    raise NotImplementedError("XRD simulation not yet implemented")

def compute_reconstruction_metrics(predicted_atoms, reference_atoms):
    """TODO: Compute structure reconstruction quality metrics."""
    raise NotImplementedError("Reconstruction metrics not yet implemented")
...
def best_cell_match(atoms, ref_cell):
    if atoms is None or ref_cell is None:
    ...
    exp_a,exp_b,exp_c = ref_cell["a"],ref_cell["b"],ref_cell["c"]
    exp_alpha,exp_beta,exp_gamma = ref_cell.get("alpha",90.0),ref_cell.get("beta",90.0),ref_cell.get("gamma",90.0)
    exp_vol = ref_cell.get("volume",0)
...
def print_cell_comparison(ref_cell, matched_params, cell_label="original"):
    if ref_cell is None or matched_params is None: return
    print(f"  ── Lattice: Target (Alexandria) vs Predicted ({cell_label}) ──")
    print(f"     {'Param':>7s}   {'Target':>10s}   {'Predicted':>10s}   {'Δ':>10s}")
    for key in ["a","b","c","alpha","beta","gamma"]:
        t,p = ref_cell.get(key), matched_params.get(key)
        if t is not None and p is not None:
            print(f"     {key:>7s}   {t:10.4f}   {p:10.4f}   {p-t:+10.4f}")
    tv,pv = ref_cell.get("volume"), matched_params.get("volume")
...
def _entry_uid(record):
    return str(record.get("jid", "unknown"))


def _row_cacheable(row):
    out = {}
    for k, v in row.items():
        if k in ("atoms", "_ds_idx"):
            out[k] = None
...
def _entry_payload_from_row(r):
    return {
        ...
        "alex_cell": {
            "a": r.get("alex_a"), "b": r.get("alex_b"), "c": r.get("alex_c"),
            "alpha": r.get("alex_alpha"), "beta": r.get("alex_beta"), "gamma": r.get("alex_gamma"),
            "volume": r.get("alex_volume"),
        },
        "pattern_matching": {
            "success": bool(r.get("pm_success")), "jid": r.get("pm_jid"),
        ...
        "diffractgpt": {
            "success": bool(r.get("dg_success")), "similarity": r.get("dg_similarity"),
            "formula": r.get("dg_formula"), "spacegroup": r.get("dg_spacegroup"),
            "num_atoms": r.get("dg_num_atoms"), "poscar": r.get("dg_poscar"),
            "atoms_dict": r.get("dg_atoms_dict"),
        },
        "best_match": {
            "method": r.get("best_method"), "similarity": r.get("best_similarity"),
            "poscar": r.get("best_poscar"), "atoms_dict": r.get("best_atoms_dict"),
        },
        "predicted_cell": {
            "a": r.get("pred_a"), "b": r.get("pred_b"), "c": r.get("pred_c"),
...
_OUTPUT_EXCLUDE = ("best_poscar","pm_poscar","dg_poscar","atoms","pm_top5","pm_atoms_dict","dg_atoms_dict","best_atoms_dict","_ds_idx")

def _write_live_outputs(results, cfg, work_dir):
    if not results:
        return
    df_live = pd.DataFrame(results)
    out_cols = [c for c in df_live.columns if c not in _OUTPUT_EXCLUDE]
    ...
    if len(poscar_entries) > 0:
        poscar_dict = {f"{r['jid']}_{r['formula']}": r["best_poscar"] for _, r in poscar_entries.iterrows()}
...
def _save_final_results(df, cfg):
    ...
    results_dir = f"alex_results_{timestamp}"
    ...
    for _, r in df.iterrows():
        safe_name = re.sub(r"[^A-Za-z0-9_-]", "_", str(r.get("jid", "unknown")))
    ...
    with open(consolidated_path, "w") as f:
        json.dump(consolidated, f, indent=2, default=_json_default)
    print(f"✓ Consolidated JSON → {consolidated_path}")

    csv_path = os.path.join(results_dir, "results.csv")
    out_cols = [c for c in df.columns if c not in _OUTPUT_EXCLUDE]
    ...
    if len(poscar_entries) > 0:
        poscar_dict = {f"{r['jid']}_{r['formula']}": r["best_poscar"] for _, r in poscar_entries.iterrows()}
        with open(cfg["output_poscars"], "w") as f:
            json.dump(poscar_dict, f, indent=2)
...
# ═══════════════════════════════════════════════════════════════════════════
def process_entry(record, two_theta_grid, headers, cfg):
    jid = record.get("jid", "unknown")
    parsed = parse_alex_entry(record)
    formula = parsed["formula"]
    elements = parsed["elements"]
    ref_cell = parsed["ref_cell"]
    intensities = parsed["intensities"]

    xrd_str = format_alex_xrd_data(intensities, two_theta_grid)
    row = {
        "jid": jid,
        "formula": formula,
        "elements": ",".join(elements),
        "num_points": len(two_theta_grid),
        "query_used": formula,
        "error": None,
    }
    for k in ["a", "b", "c", "alpha", "beta", "gamma", "volume"]:
        row[f"alex_{k}"] = ref_cell.get(k)
    ...
    try:
        ...
        matched_atoms, cell_label, matched_params = best_cell_match(row["atoms"], ref_cell)
        if matched_params is not None:
            for k in ["a","b","c","alpha","beta","gamma","volume"]:
                row[f"pred_{k}"] = matched_params[k]
            row["cell_type"] = matched_params["cell_type"]
            if cell_label != "original":
                print(f"  → Using {cell_label} cell for lattice comparison")
            print_cell_comparison(ref_cell, matched_params, cell_label)
            ...
            print_cell_comparison(ref_cell, {"a":lat.a,"b":lat.b,"c":lat.c,"alpha":lat.alpha,"beta":lat.beta,"gamma":lat.gamma,"volume":lat.volume}, "original")

        try:
            row["simulated_xrd"] = simulate_reconstructed_xrd(row["atoms"])
        except NotImplementedError:
            row["simulated_xrd"] = None
        try:
            row["reconstruction_metrics"] = compute_reconstruction_metrics(row["atoms"], None)
        except NotImplementedError:
...
def lattice_comparison(df, cfg):
    ...
    for p in params:
        mask &= df[f"alex_{p}"].notna() & df[f"pred_{p}"].notna()
    ...
    metrics = {}
    for p in params:
        exp = df_cmp[f"alex_{p}"].values.astype(float)
        pred = df_cmp[f"pred_{p}"].values.astype(float)
    ...
    for i, p in enumerate(params):
        ax = axes[i]
        exp, pred, mae, r2 = metrics[p]["exp"], metrics[p]["pred"], metrics[p]["mae"], metrics[p]["r2"]
        ...
        ax.set_aspect("equal")
        ax.set_xlabel(f"Alexandria {p} ({units[p]})")
        ax.set_ylabel(f"Predicted {p} ({units[p]})")
    ...
    fig.suptitle(f"Alexandria vs Predicted (n={n})", fontsize=14, fontweight="bold", y=1.01)
    plt.tight_layout()
...
def run_pipeline(cfg):
    ...
    print(f"Loading Alexandria dataset from: {cfg['dataset_path']}")
    ds, two_theta_grid = load_alex_dataset(
        cfg["dataset_path"],
    ...
    if cfg.get("max_abc") is not None:
        max_val = cfg["max_abc"]
        before = n_total
        def _abc_within_max(row):
            """Return True if all three cell lengths a, b, c are ≤ max_val."""
            try:
                atoms_data = json.loads(row["atoms_json"]) if isinstance(row["atoms_json"], str) else row["atoms_json"]
                return all(float(v) <= max_val for v in atoms_data["abc"][:3])
            except Exception:
                return True
        ds = ds.filter(_abc_within_max)
        n_total = len(ds)
        print(f"Filtered by a,b,c ≤ {max_val} Å: {n_total}/{before}")
    ...
    if cfg.get("max_abc"): print(f"Cell filter: Alex a,b,c ≤ {cfg['max_abc']} Å")
    print(f"{'═'*60}")
    ...
    for idx in range(n_total):
        record = ds[idx]
        jid = record.get("jid", f"entry_{idx}")
        formula = record.get("formula", "?")
        entry_id = _entry_uid(record)
        ...
        print(f"\n[{idx+1}/{n_total}] {jid} ({formula})  [elapsed: {elapsed_min:.1f}min, ETA: {eta_min:.1f}min]")
        ...
        row = process_entry(record, two_theta_grid, headers, cfg)
        row["entry_id"] = entry_id
        row["_ds_idx"] = idx
        results.append(row)
    ...
    if failed_indices:
        ...
        for df_idx in failed_indices:
            ...
            orig_ds_idx = row_data.get("_ds_idx")
            if orig_ds_idx is None:
                continue
            record_retry = ds[int(orig_ds_idx)]
            jid_retry = record_retry.get("jid", "?")
            formula_retry = record_retry.get("formula", "?")
            print(f"\n  Retrying: {jid_retry} ({formula_retry})...")
            new_row = process_entry(record_retry, two_theta_grid, headers, cfg)
            new_row["entry_id"] = row_data.get("entry_id") or _entry_uid(record_retry)
            new_row["_ds_idx"] = orig_ds_idx
    ...
    # Summary
    total = len(df)
    pm_ok = int(df["pm_success"].sum()) if "pm_success" in df.columns else 0
    dg_ok = int(df["dg_success"].sum()) if "dg_success" in df.columns else 0
    any_match = int(((df.get("pm_success", False)) | (df.get("dg_success", False))).sum())
    ...
    if len(matched) > 0:
        ...
        for _, r in matched.head(5).iterrows():
            print(f"  {str(r['jid']):20s} {str(r['formula']):15s} → sim={r['best_similarity']:.4f} ({r['best_method']}) PM_ID={r.get('pm_jid','N/A')}")
    ...
    if len(unmatched) > 0:
        ...
        for _, r in unmatched.iterrows():
            print(f"  {str(r['jid']):20s} {str(r['formula']):15s}")
    if errors > 0:
        print(f"\nERRORS ({errors}):")
        for _, r in df[df["error"].notna()].iterrows():
            print(f"  {str(r['jid']):20s} → {str(r['error'])[:80]}")
...
def parse_args():
    ...
    p = argparse.ArgumentParser(
        description="Alexandria XRD → AtomGPT.org Structure Reconstruction Benchmark",
    ...
    p.add_argument("--entry-offset", type=int, default=DEFAULTS["entry_offset"], help="Skip first N entries")
    p.add_argument("--dataset-path", default=DEFAULTS["dataset_path"], help="Local path or HuggingFace hub name")
    ...
    p.add_argument("--max-abc", type=float, default=None, help="Filter: Alex a,b,c ≤ this (Å)")
    args = p.parse_args()
    return {
